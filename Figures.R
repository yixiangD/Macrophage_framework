...
# Fig. 1C Scores and cluster annotation####
ident.scores <- list(
  "Complement & Phagocytosis" = c(
    "C1qc", "F13a1", "C1qa", "C4b", "Cfh", "C5ar1",
    "Snx2", "Tgfbr2", "Dab2", "Folr2", "Cltc", "Wwp1",
    "Cd209d", "Mrc1", "Cd209f", "Cd209g", "Cd36",
    "Ctsb", "Lgmn", "Cltc", "Cd63"
  ),
  "ECM & actin regulation" = c(
    "Cd44", "Sdc1", "Fn1",
  ...
  "Innate Immune Response" = c(
    "Tnfaip8l2", "Cyba", "Rsad2", "Anxa1", "Ifitm3",
  ...
  "Oxidative stress" = c(
    "Prdx5", "Txn1", "Gsr", "Ptgs2",
    "Ccs", "Prdx6", "Gpx4", "Sesn1",
    "Sod3", "Ltc4s"
  ),
  "ECM organization" = c(
    "Col1a1", "Nid1", "Dpt", "B4galt1",
    "Lum", "Col3a1", "Ccdc80",
    "Ramp2", "Serpinh1", "Ddr2"
  ),
  "Cycling" = c(
    "Racgap1", "Cks1b", "Stmn1", "Ran",
...
c0 <- AddModuleScore(
  object = c0, features = ident.scores,
  assay = "integrated",
  name = "Enrichment"
)
...
genes <- c(
  "S100a4", "Itgb7", "Napsa", "Cd300lg", "Adora2b", "Elane",
...
write.table(t(genes), "Monocyte.genes.noquote",
  sep = ", ", quote = FALSE,
  row.names = F, col.names = F
)

c0 <- AddModuleScore(
  object = c0, features = list(genes),
  assay = "integrated",
  name = "monocyte"
)
...
for (i in names(tg)) {
  ...
  f2b[[i]] <- FeatureScatter(
    ...
    geom_smooth(
      method = gam::gam, colour = "black", se = F,
      formula = y ~ lo(x)
    ) +
    NoLegend() +
    labs(x = titles[[i]]) +
...
df2 <- data.frame(
  ...
  "Transferred_WT" = c(
    33.85, 57.72, 15.04, 38.46,
    23.7, 46, 29, 44.4,
    52.7, NA, NA, NA,
    NA, NA, NA, NA, NA, NA
  ),
  "Transferred_KO" = c(
    40.5, 31.76, 24.23, 39.8,
    42.0, 36.1, 32.1, 23.8,
    NA, NA, NA, NA, NA,
    NA, NA, NA, NA, NA
  )
...
df1 <- data.frame(
  "WT" = c(7.79, 5.36, 3.62, 5.93, 3.53),
  "KO" = c(3.21, 4.33, 3.09, 2.9, 0.028)
)
df2 <- data.frame(
  "WT" = c(101270, 131320, 56110, 124530, 56480),
  "KO" = c(81855, 77940, 24720, 21750, 308)
)
df3 <- data.frame(
  "WT" = c(45.3, 39.6, 43.3, 64.4, 26.5),
...
# LP
p1 <- DimPlot(
  object = n2, reduction = "umap",
  pt.size = 0.1, group.by = "stage",
  cols = colors[-c(2, 6:10)]
) + NoLegend() +
  theme(
    legend.position = "none",
...
f4a <- ggplot() +
  geom_density(
    data = cm[["E14"]]@meta.data,
    aes(x = stage.score),
    color = color_hue(4)[1]
  ) +
  geom_density(
    data = cm[["P4_5"]]@meta.data,
    aes(x = stage.score),
    color = color_hue(4)[2]
  ) +
  geom_density(
    data = cm[["P30"]]@meta.data,
    aes(x = stage.score),
    color = color_hue(4)[3]
  ) +
  geom_density(
    data = cm[["P100"]]@meta.data,
    aes(x = stage.score),
    color = color_hue(4)[4]
  ) +
  theme_classic() +
...
f4b <- DimPlot(
  object = datasets[[4]], reduction = "umap",
...
f4e <- ggplot(data = t, aes(
  ...
  geom_vline(
    xintercept = quantile(c0@meta.data[c0$stage == "Late.P1", ]$Path_1)[["25%"]],
    color = "black", size = 0.5, linetype = "dashed"
  ) +
  geom_vline(
    xintercept = quantile(c0@meta.data[c0$stage == "Late.P1", ]$Path_1)[["75%"]],
    color = "black", size = 0.5, linetype = "dashed"
  ) +
...
f4f <- DimPlot(
  object = datasets[[3]], reduction = "umap",
  pt.size = 0.5, label = T, label.size = 3,
...
f4g <- VlnPlot(datasets[[3]],
  features = "Dab2", group.by = "stage",
...
css <- subset(cs, cells = cells)
f5h1 <- FeaturePlot(
  object = css, features = c("Retnla", "Ear2"),
  reduction = "umap",
  min.cutoff = "q40", max.cutoff = "q80",
  pt.size = 0.01, combine = F
)
...
f5ijk <- list()
f5ijk[[1]] <- FeatureScatter(cs,
  feature1 = "FC.UMAP_1", feature2 = "FC.UMAP_2",
  group.by = "stage", cols = colors.cs, pt.size = 0.5
)
f5ijk[[2]] <- FeatureScatter(cs,
  feature1 = "FC.UMAP_1", feature2 = "FC.UMAP_2",
  group.by = "Flow.Cluster", pt.size = 0.1
)
f5ijk[[3]] <- FeatureScatter(cs,
  feature1 = "FC.UMAP_1", feature2 = "FC.UMAP_2",
...
id <- data.frame(
  "from" = c(
    "Naive_Adipose (m)", "H.poly_Adipose (m)", "L.mono_Adipose (m)",
...
# Fig. 6C-D New Maps####
f6c <- DimPlot(
  object = c.all, reduction = "harmony.umap",
  group.by = "res_0.5",
...
f7a <- ggraph(g, layout = l) +
  ...
  guides(
    edge_alpha = guide_legend(title = "Weight"),
...
c0 <- AddModuleScore(
  object = c0, features = gg,
  assay = "integrated",
  name = "GO"
)

c0 <- AddModuleScore(
  object = c0, features = kg,
  assay = "integrated",
  name = "KEGG"
)
...
ann_colors <- list(Paths = c(
  "Phagocytic path" = "#FF7F00",
